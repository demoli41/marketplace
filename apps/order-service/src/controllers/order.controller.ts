...
export const createPaymentIntent = async (req: any, res: Response,next:NextFunction) => {
 ...
 try {
    console.log('stripe_seller_id',sellerStripeAccountId);
    const paymnetIntent=await stripe.paymentIntents.create({
        amount:customerAmount,
        currency:"gbp",
        on_behalf_of: sellerStripeAccountId || "acct_1RzeGdFn9g3BLiIg",
        payment_method_types: ["card"],
        application_fee_amount: platformFee,
        transfer_data: {
            destination: sellerStripeAccountId || "acct_1RzeGdFn9g3BLiIg",
        },
        metadata:{
            sessionId,
            userId: req.user.id,
        ...
        if(event.type === "payment_intent.succeeded") {
            ...
            if(!sessionData){
                ...
                return res.status(200).send({
                    error: "Session not found or expired, skipping order creation"
                });
            }
            ...
            if(
                ...
                for (const shop of sellerShops) {
                    ...
                    await prisma.notifications.create({
                        data: {
                            title: `Отримано нове замовлення`,
                            message: `Покупець замовив ${productTitle} з вашого магазину`,
                            creatorId:userId,
                ...
                await prisma.notifications.create({
                    data: {
                        title: `Повідомлення про нове замовлення`,
                        message: `Нове замовлення отримано від ${name}`,
                        creatorId:userId,
                        receiverId:"admin",
                        redirect_link:`http://localhost:3000/order/${sessionId}`,
                    },
...
export const updateOrderStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(!allowedStatuses.includes(deliveryStatus)){
            return res.status(400)
            .json({error:"Invalid delivery status"});
        }
