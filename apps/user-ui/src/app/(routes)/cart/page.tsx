...
const CartPage = () => {
    ...
    const couponCodeApplyHandler=async()=>{
        ...
        if(!couponCode.trim()){
            setError("Будь ласка, введіть промокод");
            return;
        }
        try {
            ...
            if(res.data.valid){
                ...
                setError(res.data.message || "Невірний промокод");
            }
        } catch (error:any) {
                setDiscountAmount(0);
                setDiscountPercent(0);
                setDiscountedProductId("");
                setError(error?.response?.data?.message || "Невірний промокод");            
        }
    };

    const createPaymentSession=async()=>{
        if(addresses?.length===0){
            toast.error("Будь ласка, додайте адресу доставки у вашому профілі");
            return;
        }
        setLoading(true);
        try {
                        ...
                        <div className='p-6 shadow-md w-full lg:w-[30%] bg-[#f9f9f9] rounded-lg'>
                            ...
                            <div className='mb-4 '>
                                ...
                                <div className='mb-4 '>
                                    ...
                                    {addresses?.length!==0 && (
                                        <select className='w-full p-2 border border-gray-200 rounded-l-md focus:outline-none focus:border-blue-500' value={selectedAddressId}
                                    ...
                                    {addresses?.length===0 && (
                                        <p className='text-sm text-slate-800'>
                                            Будь ласка, додайте адресу у вашому профілі!                                             
                                        </p>
                                    )}
                                </div>
                                <hr className='my-4 text-slate-200' />
