...
const ColorSelector = ({ control, errors }: any) => {
    ...
    return (
        <div className="mt-2">
            <label className="block font-semibold text-gray-300 mb-1">Кольори</label>
            <Controller
                name="colors"
                control={control}
                render={({ field }) => (
                    <div className="flex gap-3 flex-wrap">
                        {[...defaultColors, ...customColors].map((color) => {
                            ...
                            const isLightColor = ["#ffffff", "#ffff000"].includes(color);
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-7 h-7 p-2 rounded-md my-1 flex justify-center border-2 transition ${isSelected ? 'scale-110 border-white' : 'border-transparent'} ${isLightColor ? 'border-gray-600' : ''}`}
